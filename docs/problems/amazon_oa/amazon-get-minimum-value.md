...
- `power contains the per-round health cost for each of the n rounds, played sequentially.The armor may be applied to at most one round, reducing that round's damage by min(armor, power[i]).The player's health must remain strictly greater than 0 at all times.Return the minimum starting health that allows the player to complete all rounds.`
