"""Find Minimum Inefficiency — https://www.fastprep.io/problems/find-minimum-inefficiency

Write-up & approaches: ../../docs/problems/amazon_oa/find-minimum-inefficiency.md

Special thanks: 💐 1008th thank you to spike! 👏

  uv run python amazon_oa/find-minimum-inefficiency/solution.py   # debug one case (see CASE below)
  uv run pytest amazon_oa/find-minimum-inefficiency/              # run the test sets
"""

from harness import NotSolved, pick_case


class Solution:
    def findMinimumInefficiency(self, serverType):
        """State the time and space complexity of your approach, and explain why.

        Time:  O(?):
        Space: O(?):
        """
        raise NotSolved


if __name__ == "__main__":
    # Debug playground: set a breakpoint in findMinimumInefficiency above, then run this file.
    # Pick a case by id (ids are in cases.json).
    CASE = "example_1"
    case = pick_case(__file__, CASE)
    result = Solution().findMinimumInefficiency(*case["args"])
    print(f"case {case['id']}: args = {case['args']}")
    print(f"expected: {case['expected']}")
    print(f"got:      {result}")
