"""Next Permutation — https://www.fastprep.io/problems/amazon-next-permutation

Write-up & approaches: ../../docs/problems/amazon_oa/amazon-next-permutation.md

Return the lexicographically next permutation of nums. If the current ordering is the greatest possible, return the smallest ordering. Duplicate values are allowed.

  uv run python amazon_oa/amazon-next-permutation/solution.py   # debug one case (see CASE below)
  uv run pytest amazon_oa/amazon-next-permutation/              # run the test sets
"""

from harness import NotSolved, pick_case


class Solution:
    def nextPermutation(self, nums):
        """State the time and space complexity of your approach, and explain why.

        Time:  O(?):
        Space: O(?):
        """
        raise NotSolved


if __name__ == "__main__":
    # Debug playground: set a breakpoint in nextPermutation above, then run this file.
    # Pick a case by id (ids are in cases.json).
    CASE = "example_1"
    case = pick_case(__file__, CASE)
    result = Solution().nextPermutation(*case["args"])
    print(f"case {case['id']}: args = {case['args']}")
    print(f"expected: {case['expected']}")
    print(f"got:      {result}")
