"""Calculate Warehouse Efficiency — https://www.fastprep.io/problems/amazon-calculate-warehouse-efficiency

Write-up & approaches: ../../docs/problems/amazon_oa/amazon-calculate-warehouse-efficiency.md

The supply chain manager at one of Amazon's warehouses wants to measure the efficiency of the way parcels are shipped. The volume of each parcel is represented in the array parcelWeights. Each day, th

  uv run python amazon_oa/amazon-calculate-warehouse-efficiency/solution.py   # debug one case (see CASE below)
  uv run pytest amazon_oa/amazon-calculate-warehouse-efficiency/              # run the test sets
"""

from harness import NotSolved, pick_case


class Solution:
    def calculateWarehouseEfficiency(self, parcelWeights):
        """State the time and space complexity of your approach, and explain why.

        Time:  O(?):
        Space: O(?):
        """
        raise NotSolved


if __name__ == "__main__":
    # Debug playground: set a breakpoint in calculateWarehouseEfficiency above, then run this file.
    # Pick a case by id (ids are in cases.json).
    CASE = "example_1"
    case = pick_case(__file__, CASE)
    result = Solution().calculateWarehouseEfficiency(*case["args"])
    print(f"case {case['id']}: args = {case['args']}")
    print(f"expected: {case['expected']}")
    print(f"got:      {result}")
